...
CrawlLink.prototype.extractPage = function (url, err, response, body) {
    ...
    // console.log(this.crawledUrls[this.crawledUrls.length - 1]);
    // console.log(this.currentUrlsToCrawl[this.currentUrlsToCrawl.length - 1]);
    // console.log("current urls:  " + this.currentUrlsToCrawl.length);
    // console.log("crawled urls:  " + this.crawledUrls.length);
    // console.log(this.crawledUrls);
    // console.log(this.currentUrlsToCrawl);

    var links = this.getLinks(lastUrl, body);

    _.each(links, function (link) {
        if (self.allowCrawl(link)) {
            self.crawlUrl(link);
        }
    })
}
